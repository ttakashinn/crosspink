...
namespace dictionary_test_storage {
...
class HalFile {
 public:
  ...
  size_t write(const void* buffer, const size_t count) {
    if (!file_) return 0;
    if (dictionary_test_storage::failNextWrite) {
      dictionary_test_storage::failNextWrite = false;
      return count == 0 ? 0 : std::fwrite(buffer, 1, count - 1, file_);
    }
...
class HalStorage {
 public:
  ...
  void failNextWrite() { dictionary_test_storage::failNextWrite = true; }
  void resetFaults() { dictionary_test_storage::failNextWrite = false; }
