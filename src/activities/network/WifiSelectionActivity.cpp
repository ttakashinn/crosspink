...
void WifiSelectionActivity::attemptConnection() {
  ...
  // Set hostname so routers show "CrossPink-Reader-AABBCCDDEEFF" instead of "esp32-XXXXXXXXXXXX"
  uint8_t mac[6] = {};
  const esp_err_t macResult = esp_read_mac(mac, ESP_MAC_WIFI_STA);
  if (macResult == ESP_OK) {
    char hostname[sizeof("CrossPink-Reader-") + 12];
    snprintf(hostname, sizeof(hostname), "CrossPink-Reader-%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3],
             mac[4], mac[5]);
