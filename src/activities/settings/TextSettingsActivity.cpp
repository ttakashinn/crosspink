...
void TextSettingsActivity::onEnter() {
  ...
  fonts_.push_back({I18N.get(StrId::STR_SOURCE_SERIF), true, static_cast<uint8_t>(CrossPointSettings::SOURCESERIF)});
  fonts_.push_back({I18N.get(StrId::STR_SOURCE_SANS), true, static_cast<uint8_t>(CrossPointSettings::SOURCESANS)});
  if (registry_) {
