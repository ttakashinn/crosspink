...
void silentRestartToVanNhanSoSettings() {
  if (deepSleepInProgress) return;
#if FREEINK_CAP_TOUCH
  if (finishWifiSessionWithoutRestart()) return;
#endif
...
void setup() {
  ...
  LOG_DBG("MAIN", "Starting CrossPink version " CROSSPOINT_VERSION);
  ...
  } else if (resume == BootResume::Silent && snapshotTarget == SILENT_REBOOT_TARGET_VANNHANSO_SETTINGS) {
...
void loop() {
  ...
  // Avoid a full reader-page repaint merely because charging state changed;
  // the reader status bar will pick it up on the next normal page render.
