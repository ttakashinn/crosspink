...
`scripts/render_lab.py` tạo một SD root tạm cho từng case, chép fixture vào `/books`, cấu hình reader từ profile trong `expected-manifest.json`, rồi chạy native firmware. `EpubReaderActivity` mở sách qua `HalStorage`, resolve `spine href + anchor`, layout section, render page bằng font/cache/image decoder thật và xuất:
...
- `result.json`: viewport, spine/page, visible-text offset, cache state, font/settings, timing và heap host được simulator báo cáo.
...
- `full`: 9 checkpoint trên 3 profile; checkpoint stress chạy cả cold và warm, tổng cộng 30 case.
...
Review cả ảnh và `result.json`, đặc biệt `page_count`, `page_index`, `visible_text_offset`, kích thước logic và trạng thái grayscale plane.
